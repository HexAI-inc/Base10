...
async def create_classroom(
    ...
    Teachers use this to organize students into groups.
    Returns a unique join code for students.
    """
...
async def list_classrooms(
    ...
    Returns classrooms with student counts.
    """
    classrooms = db.query(
        Classroom,
...
@router.post("/classrooms/join", status_code=status.HTTP_200_OK)
async def join_classroom(
    ...
    Students enter the code from their teacher to auto-enroll.
    """
    classroom = db.query(Classroom).filter(
        Classroom.join_code == join_data.join_code,
    ...
    if not classroom:
        raise HTTPException(
            ...
            detail="Classroom not found or inactive"
        )
    
    # Check if already joined
    already_joined = current_user in classroom.students
    if already_joined:
        return {"message": "Already enrolled in this classroom"}
